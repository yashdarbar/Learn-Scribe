...
- Return only the JSON array, no other text or formatting`;
    ...
    try {
      ...
      if (!Array.isArray(flashcards) || flashcards.length !== 5) {
        throw new Error(`Expected exactly 5 flashcards, received ${flashcards?.length || 0}`);
      }
      ...
      return {
        success: false,
        error: `Failed to parse flashcard data: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`
      };
    }
